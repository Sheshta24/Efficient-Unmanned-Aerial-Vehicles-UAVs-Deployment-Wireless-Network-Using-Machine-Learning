...
def uav_deployment_optimization(num_users, area_size):
    ...
    prob += pulp.lpSum(p[i] for i in range(num_clusters)) + 10* pulp.lpSum(C[i, k] for i in range(num_clusters) for k in range(num_clusters) if i != k)
    ...
    # Limit number of connectivity links to 3 per UAV
    for i in range(num_clusters):
        prob += pulp.lpSum(C[i, k] for k in range(num_clusters) if i != k) <= 3
...
# Post-processing function for user assignments and connectivity links removal
def greedy_post_processing(uavs, user_assignments, original_uav_indices, user_locations, centroids, alt_min, alt_max, C):
    while True:
        impact_scores = []
        ...
        for user in removed_users:
            ...
            for i, uav in enumerate(uavs):
                if i == least_impact_uav_index:
                    continue
                if can_assign_user_to_uav(user, i, uavs, user_locations, centroids, alt_min, alt_max):
...
# Helper function to check if a user can be assigned to a UAV
def can_assign_user_to_uav(user_index, uav_index, uavs, user_locations, centroids, alt_min, alt_max):
...
# Main script
area_size = 500
user_counts = [100, 200, 300, 400, 500, 600, 700, 800]
results = {}
...
for num_users in user_counts:
    ...
    uavs, user_assignments, original_uav_indices = greedy_post_processing(
        uavs, user_assignments, original_uav_indices, user_locations, centroids, alt_min, alt_max, C
    )
...
# Plotting the results for UAVs vs Users
